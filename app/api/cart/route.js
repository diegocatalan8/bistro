...
export async function GET(){
  ...
  try{
    ...
    const query = 'SELECT  C.ID, C.VARIATION_ID, C.ORDER_ID, C.QUANTITY, C.NOTE, O.ID, O.DESCRIPTION, V.NAME, V.DESCRIPTION, V.PRICE FROM TBL_PRODUCT_CART AS C INNER JOIN TBL_VARIATION AS V ON C.VARIATION_ID = V.ID INNER JOIN TBL_ORDER AS O ON C.ORDER_ID = O.ID WHERE C.STATUS = TRUE';
