...
function PageOrder({ params }) {
  ...
  const searcherMotor = products.filter((item) => {
    ...
    let upperCaseProductName = item.product_name
      ? item.product_name.toUpperCase()
      : '';
    let upperCaseDescription = item.description
  ...
  return loading ?
    ...
    :(<div className='Container relative z-10  w-full h-full flex flex-row'>
      <section
        className={`Section px-5 pt-6 h-full w-[90%] ${
          isCartOpen ? 'md:w-[50%]' : 'md:w-[95%]'
        }  lg:w-[65%] xl:w-[70%] flex flex-col `}
      >
        {/**SEARCH BAR */}
        ...
        <div className=' min-h-[70px] max-w-full my-3 overflow-hidden'>
          <div className='flex flex-row justify-start items-center w-full h-full overflow-x-scroll'>
            <button
              ...
              className={`rounded-lg w-fit h-[90%] md:min-h-[60%] lg:h-[60%] px-2 mr-3 ${
                categoryActive === 'Todos'
                  ? 'bg-[#2d69ff] text-white font-semibold shadow-md'
                  : 'bg-white'
              }`}
            >
              Todos
            ...
            {categories.map((category) => (
              <button
                ...
                className={`rounded-lg w-fit h-[90%] md:min-h-[60%] lg:h-[60%] px-3 mr-3 ${
